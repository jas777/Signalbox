...
public class BlockHuMainSignalHeadBottom extends BaseSignal implements HasVariant, HasParts {
    ...
    public BlockHuMainSignalHeadBottom() {
        super("hu_main_head_bottom", Material.IRON);
    ...
    public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
        worldIn.setBlockState(pos.up(), ModBlocks.HU_MAIN_SIGNAL_HEAD_TOP.getDefaultState().withProperty(BlockHuMainSignalHeadTop.FACING, state.getValue(FACING)).withProperty(BlockHuMainSignalHeadTop.SIGNAL_VARIANT, state.getValue(SIGNAL_VARIANT)));
    }

    @Override
    public void updateParts(World worldIn, BlockPos pos, IBlockState state) {

        if (!(worldIn.getBlockState(pos.up()).getBlock() instanceof BlockHuMainSignalHeadTop)) {
            breakBlock(worldIn, pos, state);
        }

        worldIn.setBlockState(pos.up(), worldIn.getBlockState(pos.up()).withProperty(BlockHuMainSignalHeadTop.SIGNAL_VARIANT, state.getValue(SIGNAL_VARIANT)).withProperty(BlockHuMainSignalHeadTop.ACTIVE, state.getValue(ACTIVE)));

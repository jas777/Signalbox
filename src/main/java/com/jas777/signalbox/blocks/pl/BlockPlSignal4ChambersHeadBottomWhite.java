...
public class BlockPlSignal4ChambersHeadBottomWhite extends BaseSignal implements HasVariant, HasParts {
    ...
    public BlockPlSignal4ChambersHeadBottomWhite() {
        super("pl_signal_4chambers_head_bottom_white", Material.IRON);
    ...
    public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
        worldIn.setBlockState(pos.up(), ModBlocks.PL_SIGNAL_4_HEAD_MIDDLE_WHITE.getDefaultState().withProperty(BlockPlSignal4ChambersHeadMiddleWhite.FACING, state.getValue(FACING)).withProperty(BlockPlSignal4ChambersHeadMiddleWhite.SIGNAL_VARIANT, state.getValue(SIGNAL_VARIANT)),2 );
    ...
    public void updateParts(World worldIn, BlockPos pos, IBlockState state) {
        ...
        if (!(worldIn.getBlockState(pos.up()).getBlock() instanceof BlockPlSignal4ChambersHeadMiddleWhite)) {
        ...
        worldIn.setBlockState(pos.up(), worldIn.getBlockState(pos.up()).withProperty(BlockPlSignal4ChambersHeadMiddleWhite.SIGNAL_VARIANT, state.getValue(SIGNAL_VARIANT)).withProperty(BlockPlSignal4ChambersHeadMiddleWhite.ACTIVE, state.getValue(ACTIVE)), 2);
